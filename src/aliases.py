...
VOCABULARY = {
    ...
    # ---------------------------------------------------------
    'pie': 'class',  # Класс - это один большой пирог
    'chef': 'def',  # Шеф-повар реализует рецепт
    ...
    'sugar': 'super',  # Сахар - это то, что делает всё слаще/лучше, как и супер-класс
    ...
    'stir': 'iter',  # Помешивать = итерировать
    ...
    'giftbox': 'tuple',  # Подарочная коробка - запечатана, неизменяема
    'buffet': 'list',  # Буфет - множество блюд, можно брать и добавлять
    'assortment': 'set',  # Коробка ассорти - набор разных уникальных сладостей
    ...
    'parfait': 'complex',  # Парфе - сложный многослойный десерт
    'unwrap': 'open',  # Развернуть упаковку (открыть файл)
    'peel': 'strip',  # Почистить от лишнего (пробелов в начале и конце строки)
    'pure': 'abs',  # Чистый, абсолютный (модуль числа)
    'chop': 'split',  # Порубить/разделить (строку)
    'crust': 'repr',  # Корочка = внешний вид объекта
    'whip': 'sum',  # Взбить всё вместе (сумма)
    'crumble': 'divmod',  # Раскрошить (деление с остатком)
    'garnish': 'format',  # Украсить блюдо (форматирование строки)
    'startsweet': 'startswith',  # Сладкое начало (просто созвучное)
    'endsweet': 'endswith',  # Сладкий конец (просто созвучное)
    'popcorn': 'pop',  # Вынуть один элемент, как попкорн (просто созвучное)
    'mash': 'hash',  # Размять в однородную массу (хэш-значение)

    # ---------------------------------------------------------
    # Асинхронность
    # ---------------------------------------------------------
    'preheat': 'async',  # Предварительный разогрев духовки
    'simmer': 'await',  # Томить на медленном огне, ждать готовности

    # ---------------------------------------------------------
    # Модули и пакеты
    # ---------------------------------------------------------
    '__cookbook__': '__name__',  # Книга рецептов (имя модуля)
    '__pantry__': '__file__',  # Кладовая (путь к файлу)
    '__kitchen__': '__main__',  # Кухня (главный модуль)
    '__ingredients__': '__dict__',  # Ингредиенты (атрибуты объекта)
    '__platter__': '__str__',  # Подача на блюде (строковое представление)
    '__fresh__': '__new__',  # Свежий (новый) экземпляр класса

    # ---------------------------------------------------------
    # Исключения и ошибки
    # ---------------------------------------------------------
    'MissingIngredient': 'KeyError',  # Отсутствует ингредиент
